...
public class Main{
    ...
    public static void moverAgente(){
        ...
        if(automatico){
            agente.calcularMovimento(tamanhoMapa, movimentosFeitos);
    ...
    public static void gerarEntidadesFixas(){
        if(agente == null) agente = new Agente(xInicialAgente, yInicialAgente);
        else{
            ...
            agente = new Agente(xInicialAgente, yInicialAgente);
    ...
    public static void calcularMapaSensacoes(){
        ...
        for(int i = 0; i < wumpus.size(); i++){//percorrer cada monstro da lista
            for(int contador = 0; contador < 4; contador++){
                ...
                if((x >= 0) && (x < tamanhoMapa) && (y >= 0) && (y < tamanhoMapa) && wumpus.get(i).getStatus()){//posição dentro do mapa
                    if(!mapaSensacoes[x][y].equalsIgnoreCase(wumpus.get(i).getSensacao())){
        ...
        //percorrer cada poço
        for(int contador2 = 0; contador2 < quantidadePoco; contador2++){
            //calcular direções
            for(int contador = 0; contador < 4; contador++){
    ...
    public static void verificarSensacoes(){
        ...
        for(int i = 0; i < wumpus.size(); i++){
            if(agente.getSensacoes().contains(wumpus.get(i).getSensacao()) && (agente.getFlechas() > 0)){       
                System.out.println(); 
                retornoAtirar = agente.atirar(wumpus.get(i), tamanhoMapa);
                indiceWumpus = i;
                break;
            }
        }
